...
namespace ColPack
{
	...
	int BipartiteGraphPartialColoring::PartialDistanceTwoRowColoring_OMP()
	{
		...
		while(i_NumOfVerticesToBeColored!=0) {
			colNext = colMax = 0;
#pragma omp parallel for default(none) schedule(dynamic) shared(cout, i_NumOfVerticesToBeColored, vi_VerticesToBeColored) firstprivate(colNext, colMax, vi_forbiddenColors)


        	for(int i=0; i<i_NumOfVerticesToBeColored; i++) {
				int v = vi_VerticesToBeColored[i];
				for (int w=m_vi_LeftVertices [v]; w<m_vi_LeftVertices [v+1]; w++ ) {//nets()
					for (int x=m_vi_RightVertices [m_vi_Edges [w]]; x<m_vi_RightVertices [m_vi_Edges [w]+1]; x++ ) {//vtx()
				...
				*/
				...
				/*
				//second from last
				int i_cadidateColor = m_i_LeftVertexColorCount;
				{
					while(vi_forbiddenColors[i_cadidateColor]==v && i_cadidateColor >= 0) i_cadidateColor--;
					if(i_cadidateColor < 0) i_cadidateColor = m_i_LeftVertexColorCount + 1;
				}
				*/
				...
				int i_cadidateColor;
				if( v % 2 == 0 ){
					i_cadidateColor = m_i_LeftVertexColorCount;
					while(vi_forbiddenColors[i_cadidateColor]==v && i_cadidateColor >= 0) i_cadidateColor--;
					if(i_cadidateColor < 0) i_cadidateColor = m_i_LeftVertexColorCount + 1;
				}
				else{
				...
				*/
				...
				//colNext, colMax inicializados en 0
				int i_cadidateColor = colNext;
				while( i_cadidateColor < vi_forbiddenColors.size() && vi_forbiddenColors[i_cadidateColor]==v) 
					i_cadidateColor++;

				if(i_cadidateColor > colMax){
					i_cadidateColor = 0;
					while(vi_forbiddenColors[i_cadidateColor]==v) i_cadidateColor++;
				}

				colMax = max(colMax, i_cadidateColor);
				colNext = min(i_cadidateColor+1,colMax/3+1);
...
#pragma omp critical
	...
	int BipartiteGraphPartialColoring::PartialDistanceTwoRowColoring_6_OMP()
	{
		if(CheckVertexColoring("ROW_PARTIAL_DISTANCE_TWO"))
		{
			return(_TRUE);
		...
		while(i_NumOfVerticesToBeColored!=0) {
			...
			int i_cadidateColor=0;
#ifdef _OPENMP
#pragma omp parallel for default(none) schedule(dynamic) shared(cout, i_LeftVertexCount, i_NumOfVerticesToBeColored, vi_VerticesToBeColored) firstprivate(i_cadidateColor, vi_forbiddenColors)
#endif
        	...
        	for(int i=0; i<i_NumOfVerticesToBeColored; i++) {
				...
				for (int u=m_vi_RightVertices [m_vi_Edges [v]]; u<m_vi_RightVertices [m_vi_Edges [v]+1]; u++ ) {//u E vtxs(v)
					if ( m_vi_LeftVertexColors [m_vi_Edges [u]] != _UNKNOWN or vi_forbiddenColors[ m_vi_LeftVertexColors [m_vi_Edges [u]]] != u) {
						while(vi_forbiddenColors[i_cadidateColor]!=v) i_cadidateColor++;
						vi_forbiddenColors [m_vi_LeftVertexColors [m_vi_Edges [u]]] = u;
					}
				}
...
#pragma omp critical
	...
	int BipartiteGraphPartialColoring::PartialDistanceTwoRowColoring() {
		...
		//return BipartiteGraphPartialColoring::PartialDistanceTwoRowColoring_6_OMP();
#else
